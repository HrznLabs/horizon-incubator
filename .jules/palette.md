...
## 2026-07-02 - Accessible CSS Pseudo-element Content
**Learning:** When using CSS pseudo-elements (`::before`, `::after`) for decorative characters (e.g. `content: "→"`), screen readers may erroneously read them aloud.
**Action:** Use the modern CSS alternative text syntax with an empty string (`content: "\2192" / "";`) to hide them accessibly, providing a standard fallback (`content: "\2192";`) immediately before it for backward compatibility.
