...
## 2026-07-12 - Accessibly Hiding Pseudo-element Tooltips
**Learning:** When implementing CSS tooltips via pseudo-elements using "content: attr(aria-label);" on elements that already possess an "aria-label", screen readers redundantly announce the label twice.
**Action:** Prevent redundant screen reader announcements by adding "content: attr(aria-label) / '';" immediately after the standard fallback to accessibly hide the pseudo-element's content.
