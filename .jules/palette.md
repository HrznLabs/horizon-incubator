...
## 2026-07-16 - Prevent screen reader redundancy on CSS tooltips
**Learning:** Using `content: attr(aria-label);` for tooltips on elements that already have an `aria-label` causes screen readers to redundantly announce the label twice.
**Action:** Always append `content: attr(aria-label) / "";` immediately after the standard fallback to accessibly hide the pseudo-element's content from screen readers.
