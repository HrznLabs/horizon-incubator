...
## 2025-02-06 - Add custom scrollbar styling to dark-themed Mermaid containers
**Learning:** Default browser scrollbars often contrast harshly against dark-themed components like horizontally scrollable `.mermaid` diagrams, degrading the visual experience.
**Action:** Always implement custom scrollbar styles (using both `scrollbar-color` and `::-webkit-scrollbar` properties) that match the application's accent colors for horizontally scrollable containers to maintain visual polish.
