...
## 2025-05-24 - Prevent Information Leakage in Catch Blocks
**Vulnerability:** Raw error objects passed to `console.error(err)` can expose stack traces and internal application state in the client console.
**Learning:** Even static HTML pages with client-side scripts are susceptible to information leakage when uncaught exceptions or promises rejections are naively logged directly to the browser console. Modifying inline scripts on pages with strict CSPs requires recalculating the corresponding `script-src` SHA-256 hashes.
**Prevention:** Always replace raw error objects with generic error messages when logging to the console to prevent information leakage, such as replacing `console.error(err)` with `console.error('Failed to render diagram.')`.
