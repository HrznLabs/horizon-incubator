...
## 2026-04-09 - [DOM XSS via innerHTML in Static Specifications]
**Vulnerability:** The anchor link script in a static HTML specification used `innerHTML` to update the icon of the copy button (`btn.innerHTML = '<span aria-hidden="true">✅</span>';`). While the input was seemingly static in this context, using `innerHTML` is an XSS anti-pattern and a potential sink if the input ever becomes dynamic or user-controlled.
**Learning:** Even in static, non-user-facing files like specifications, `innerHTML` should be avoided for simple DOM updates. It creates unnecessary risk and violates the principle of defense in depth. Refactoring these to use safer DOM manipulation methods (`document.createElement`, `textContent`) is a straightforward way to eliminate the sink entirely.
**Prevention:** Always use `textContent` instead of `innerHTML` when setting text content, and use `document.createElement` along with `appendChild` for constructing DOM elements, even for simple, seemingly static string replacements.
