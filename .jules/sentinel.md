...
## 2026-07-08 - Fix Clickjacking bypass via iframe sandboxing
**Vulnerability:** The top.location = self.location anti-clickjacking script can be bypassed if the page is embedded in a sandboxed iframe (sandbox="allow-scripts" without allow-top-navigation).
**Learning:** Sandboxed iframes can block frame-busting scripts that rely on simple assignment to top.location.
**Prevention:** Use window.top.location.replace(window.self.location.href) for robust anti-clickjacking, and always ensure newly injected script's SHA-256 hash is updated in the CSP meta tag.
