...
## 2026-05-16 - Framebusting Script for Clickjacking Protection
**Vulnerability:** Static HTML files served directly (e.g., via IPFS, S3 buckets without headers, or local files) cannot rely on `X-Frame-Options` or Content-Security-Policy `frame-ancestors` directives delivered via `<meta>` tags to prevent clickjacking. Browsers explicitly ignore these directives when delivered via meta tags.
**Learning:** To protect static HTML documents from being embedded in malicious iframes when server-side HTTP headers are unavailable, a fallback "frame-busting" JavaScript approach must be employed. This approach involves a script checking if `window.top` is different from `window.self`, and if so, forcing the top-level window to redirect to the current document's URL.
**Prevention:** Implement a secure frame-busting script inline in the `<head>` of static HTML documents and ensure its SHA-256 base64-encoded hash is included in the CSP `script-src` directive to allow execution.
