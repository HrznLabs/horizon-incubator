...
## 2026-07-20 - Frame-Busting Bypass via History Manipulation
**Vulnerability:** The existing anti-clickjacking code used `top.location = self.location`, which can be bypassed by attackers manipulating the browser history (e.g., using `onbeforeunload` or HTTP 204) to trap the navigation and keep the frame loaded.
**Learning:** Standard navigation assignment for frame-busting is vulnerable to history-based attacks.
**Prevention:** Always use `window.top.location.replace(window.self.location.href)` to replace the current history state, preventing the attacker from canceling the navigation and ensuring the frame breaks out successfully.
