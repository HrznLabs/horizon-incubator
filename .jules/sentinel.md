...
## 2025-01-01 - Fixing Information Leakage in JS Logs
**Vulnerability:** The application was logging raw error objects (e.g., `console.error(err)`) to the browser console during diagram rendering failures. This could expose internal stack traces or application state to the client side.
**Learning:** To prevent information leakage in client-side scripts, never log raw error objects which can expose sensitive internals.
**Prevention:** Always replace raw error logging with safe, generic error messages (e.g., `console.error('Failed to render diagram.');`).
