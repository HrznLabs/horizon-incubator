...
## 2026-07-24 - Upgrading Static HTML Clickjacking Defense
**Vulnerability:** A static HTML file used `top.location = self.location` as an anti-clickjacking defense. This pattern is vulnerable to bypasses because an attacker framing the page can trap the navigation attempt (e.g., using `onbeforeunload` events or an HTTP 204 response), keeping the user trapped in the malicious iframe.
**Learning:** For static HTML where HTTP headers (`X-Frame-Options` / `frame-ancestors`) cannot be reliably set (as meta tags do not support them), the inline script must securely break out of frames. Using `window.top.location.replace(window.self.location.href)` prevents the attacker from trapping the navigation because it replaces the current history state rather than initiating a new navigation that can be intercepted.
**Prevention:** Always use `window.top.location.replace()` in inline frame-busting scripts to ensure navigation cannot be trapped. When updating this script, always recount its SHA-256 hash and update the corresponding entry in the `<meta>` Content-Security-Policy to ensure the script executes.
