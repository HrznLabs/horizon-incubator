...
## 2026-05-16 - [Clickjacking Prevention in Static HTML]
**Vulnerability:** Static HTML specification files lacked protection against clickjacking. Browsers ignore `X-Frame-Options` or `frame-ancestors` (CSP) directives when they are specified in `<meta>` tags rather than HTTP response headers. Consequently, an attacker could embed the document within an iframe on a malicious site.
**Learning:** To prevent clickjacking in statically served HTML files without server-side HTTP header configuration, an inline frame-busting JavaScript snippet is required. Relying on `<meta>` tag CSP frame directives is insufficient for clickjacking protection.
**Prevention:** Always inject a frame-busting snippet (e.g., `if (window.top !== window.self) window.top.location.replace(window.self.location.href);`) within an inline `<script>` tag at the start of the document's `<head>`. Crucially, remember to simultaneously calculate its SHA-256 hash and append it to the `script-src` directive of the document's Content-Security-Policy to ensure the script is not blocked by the browser.
## 2026-05-28 - Missing CSP Hash for Inline Script
...
## 2026-07-01 - OWASP CSS/JS Anti-Clickjacking Pattern for Static HTML
...
## 2025-02-28 - Preventing Clickjacking Navigation Traps
**Vulnerability:** The standard OWASP frame-busting script (`top.location = self.location`) is vulnerable to navigation trapping where an attacker uses `onbeforeunload` or `204 No Content` to prevent the browser from executing the redirect and breaking out of the frame.
**Learning:** Using `window.top.location.replace(window.self.location.href)` is significantly more secure because `replace()` modifies the history state, making it harder for the attacker to trap the victim, and doesn't pollute the browser history. Additionally, when modifying inline scripts protected by a strict CSP, any change (even a comment) changes the SHA-256 hash. The new hash must be mathematically derived from the *exact* new script string (including all whitespace/newlines) and updated in the CSP meta tag simultaneously, or the browser will block the script and potentially break the page (e.g., if the script was meant to unhide the `body`).
**Prevention:** Always use `window.top.location.replace()` for frame-busting instead of assignment. When modifying CSP-protected files, dynamically calculate the new SHA-256 hash in the deployment script to guarantee accuracy, rather than hardcoding pre-calculated values which are prone to hallucination or whitespace mismatch errors.
