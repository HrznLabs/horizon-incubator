...
## 2026-07-29 - Upgraded anti-clickjacking defense in RidesVertical_Complete_Spec.html
**Vulnerability:** The anti-clickjacking script used `top.location = self.location`, which is vulnerable to bypasses via history manipulation (e.g., attackers trapping the navigation using `onbeforeunload` events or HTTP `204 No Content`).
**Learning:** Legacy frame-busting scripts using direct assignment can be bypassed. `window.top.location.replace()` should be used instead.
**Prevention:** Always use `window.top.location.replace(window.self.location.href)` for frame-busting to prevent navigation trapping and avoid polluting the user's browser history. Remember to update the CSP hash for the modified inline script.
