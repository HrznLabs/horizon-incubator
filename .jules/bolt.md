...
## 2026-10-24 - Avoid transition: all in CSS
**Learning:** Using `transition: all` in CSS is a performance anti-pattern that causes unnecessary style recalculations and layout thrashing during state changes.
**Action:** Always specify explicit transition properties (e.g., `transition: opacity 0.3s ease`) to optimize rendering performance.
