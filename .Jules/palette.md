...
## 2025-06-17 - [Accessible Clipboard Feedback]
**Learning:** When building interactive copy-to-clipboard buttons in standalone HTML specifications, visual feedback (like temporarily changing the icon to a checkmark) is insufficient for users relying on screen readers. Without an audible announcement, screen reader users are left unsure if the copy action succeeded.
**Action:** When implementing copy-to-clipboard functionality, temporarily update the button's `aria-label` attribute to indicate success (e.g., "Copied to clipboard") concurrently with the visual feedback. Ensure the original `aria-label` is stored and reliably restored after a brief timeout, or completely removed if the attribute was not originally present, to maintain a clean accessibility tree.
