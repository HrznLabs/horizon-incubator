...
## 2025-06-16 - [Audible Feedback for Temporary Visual States]
**Learning:** For interactive elements like copy-to-clipboard buttons, changing only the visual state (e.g., `innerHTML` to '✅') completely isolates screen reader users from feedback about the action's success. Furthermore, when temporarily mutating `aria-label` to provide this feedback, failing to handle `null` original states will leave the element with an invalid `aria-label="null"` attribute.
**Action:** When implementing temporary visual feedback for interactive elements, ensure the `aria-label` is also updated to provide audible confirmation (like "Link copied"). Always cache the original state, and check if it was `null` to use `removeAttribute()` instead of blindly using `setAttribute()` when reverting.
